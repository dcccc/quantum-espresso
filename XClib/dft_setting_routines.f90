...
CONTAINS
  ...
  SUBROUTINE xclib_set_auxiliary_flags( isnonlocc )
    ...
    USE dft_setting_params,  ONLY: iexch, icorr, igcx, igcc, imeta, imetac, &
                                   ...
                                   is_libxc, exx_term, rvv10_b, rvv10_c 
#if defined(__LIBXC)
    USE xc_f03_lib_m
#endif
    !
    ...
    DO iid = 1, 6
      IF ( is_libxc(iid) ) THEN
        ...
        ENDDO
        !
        IF ( libxc_flag(11)==1 ) THEN
          CALL xc_f03_nlc_coef( xc_func, rvv10_b,  rvv10_c  )
          WRITE(stdout,'(5x,"rvv10 b value from libxc",f12.7)' )  rvv10_b
          WRITE(stdout,'(5x,"rvv10_c value from libxc",f12.7)' )  rvv10_c
        ENDIF 
        !
          ...
          IF ( (MOD(iid,2)==1 .OR. ((MOD(iid,2)==0) .AND. &
             ...
             IF (exx_fraction==0.d0) THEN
               IF (libxc_flag(9) == 1) THEN
                 ...
                 IF (screening_parameter==0.d0) THEN 
                    screening_parameter = omega
                    WRITE(stdout,'(5x,"screening_parameter from libxc ",f12.7)' )  screening_parameter
                    ENDIF
                 IF (exx_fraction==0.d0) THEN 
                    exx_fraction = alpha + beta   
                    WRITE(stdout,'(5x,"exx_fraction from libxc",f12.7)' )  exx_fraction
                    ENDIF
                 IF (exx_fraction_lr==0.d0) THEN 
                    exx_fraction_lr = -1.d0*beta  
                    WRITE(stdout,'(5x,"exx_fraction_lr from libxc",f12.7)' )  exx_fraction_lr
                    ENDIF
                ELSE
...
!   END SUBROUTINE init_dft_exxrpa
  ...
  !-----------------------------------------------------------------------
  SUBROUTINE xclib_set_exx_fraction( exx_fraction_,  exx_fraction_lr_ )
    !! Impose input parameter as exact exchange fraction value
    USE kind_l,             ONLY: DP
    USE dft_setting_params, ONLY: exx_fraction, exx_fraction_lr
    IMPLICIT NONE
    REAL(DP), INTENT(IN) :: exx_fraction_
    REAL(DP), INTENT(IN) :: exx_fraction_lr_
    !! Imposed value of exact exchange fraction
    exx_fraction = exx_fraction_
    exx_fraction_lr = exx_fraction_lr_
    WRITE( stdout,'(5x,a,f6.2)') 'EXX fraction changed: ', exx_fraction
    WRITE( stdout,'(5x,a,f6.2)') 'EXX fraction_lr : ', exx_fraction_lr
    RETURN
  ...
  SUBROUTINE xclib_init_libxc( xclib_nspin, domag )
    ...
    INTEGER :: i, ii, iid, iexx, iscr,iexx_sr, iexx_lr, ip, nspin0, iflag, family
      ...
      IF (is_libxc(iid)) THEN
        ...
        IF (exx_term==iid .AND. exx_fraction>0.d0 .AND. n_ext_params(iid)/=0 ) THEN
          ...
          DO i = 0, n_ext_params(iid)-1
            ...
            ENDDO
            DO ii = 1, 6
              IF ( matches( lxc_exx_sr_desc(ii), pdesc ) ) iexx_sr = i
            ENDDO

              IF ( matches( lxc_exx_lr_desc(1), pdesc ) ) iexx_lr = i

          ENDDO
          !
          IF      (iexx /= -1 .AND. iexx_sr==-1 .AND. iexx_lr==-1  ) THEN
            IF (iexx/= 0) CALL set_libxc_ext_param( iid, iexx, exx_fraction )
            WRITE(stdout,'(5x,"exx_fraction to libxc ",f12.7)' )  exx_fraction
          ELSE IF (iexx /= -1  .AND. iexx_sr/=-1  .AND. iexx_lr == -1 ) THEN
            CALL set_libxc_ext_param( iid, iexx   , exx_fraction + exx_fraction_lr )
            WRITE(stdout,'(5x,"exx_fraction to libxc ",f12.7)' )  exx_fraction + exx_fraction_lr
            CALL set_libxc_ext_param( iid, iexx_sr, exx_fraction_lr*-1.d0 )
            WRITE(stdout,'(5x,"exx_fraction_sr to libxc ",f12.7)' )  exx_fraction_lr*-1.d0
          ELSE IF (iexx /= -1  .AND. iexx_sr==-1 .AND. iexx_lr /= -1) THEN
            CALL set_libxc_ext_param( iid, iexx   , exx_fraction    )
            WRITE(stdout,'(5x,"exx_fraction to libxc ",f12.7)' )  exx_fraction
            CALL set_libxc_ext_param( iid, iexx_lr, exx_fraction_lr )
            WRITE(stdout,'(5x,"exx_fraction_lr to libxc ",f12.7)' )  exx_fraction_lr
          ELSE IF (iexx == -1 .AND. iexx_sr/=0 .AND. iexx_lr /= -1  ) THEN
            CALL set_libxc_ext_param( iid, iexx_sr, exx_fraction    )
            WRITE(stdout,'(5x,"exx_fraction_sr to libxc ",f12.7)' )  exx_fraction
            CALL set_libxc_ext_param( iid, iexx_lr, exx_fraction_lr + exx_fraction )
            WRITE(stdout,'(5x,"exx_fraction_lr to libxc ",f12.7)' )  exx_fraction_lr + exx_fraction
          ELSE IF (iexx/=  -1 .AND. iexx_sr==-1  .AND. iexx_lr == -1 ) THEN
            CALL set_libxc_ext_param( iid, iexx,    exx_fraction )
            WRITE(stdout,'(5x,"exx_fraction to libxc ",f12.7)' )  exx_fraction
          ELSE IF (iexx==  -1 .AND. iexx_sr/=-1  .AND. iexx_lr == -1 ) THEN
          ...
          IF (iscr/= 0 .AND. screening_parameter>0.d0) &
            ...
            WRITE(stdout,'(5x,"screening_parameter to libxc ",f12.7)' )  screening_parameter
