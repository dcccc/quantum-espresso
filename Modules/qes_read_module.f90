...
MODULE qes_read_module
  ...
  SUBROUTINE qes_read_hybrid(xml_node, obj, ierr )
    ...
    END IF
    !
    tmp_node_list => getElementsByTagname(xml_node, "exx_fraction_lr")
    tmp_node_list_size = getLength(tmp_node_list)
    !
    IF (tmp_node_list_size > 1) THEN
        IF (PRESENT(ierr) ) THEN
           CALL infomsg("qes_read:hybridType","exx_fraction_lr: too many occurrences")
           ierr = ierr + 1
        ELSE
           CALL errore("qes_read:hybridType","exx_fraction_lr: too many occurrences",10)
        END IF
    END IF
    !
    IF (tmp_node_list_size>0) THEN
      obj%exx_fraction_lr_ispresent = .TRUE.
      tmp_node => item(tmp_node_list, 0)
      CALL extractDataContent(tmp_node, obj%exx_fraction_lr , IOSTAT = iostat_)
      IF ( iostat_ /= 0 ) THEN
         IF ( PRESENT (ierr ) ) THEN
            CALL infomsg("qes_read:hybridType","error reading exx_fraction_lr")
            ierr = ierr + 1
         ELSE
            CALL errore ("qes_read:hybridType","error reading exx_fraction_lr",10)
         END IF
      END IF
    ELSE
       obj%exx_fraction_lr_ispresent = .FALSE.
    END IF
