...
MODULE qes_write_module
  ...
  !
   ...
   SUBROUTINE qes_write_hybrid(xp, obj)
     ...
     END IF
     IF (obj%exx_fraction_lr_ispresent) THEN
        CALL xml_NewElement(xp, "exx_fraction_lr")
           CALL xml_addCharacters(xp, obj%exx_fraction_lr, fmt='s16')
        CALL xml_EndElement(xp, "exx_fraction_lr")
     END IF
