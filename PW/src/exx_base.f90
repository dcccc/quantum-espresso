...
MODULE exx_base
 ...
 CONTAINS
  ...
  SUBROUTINE g2_convolution( ngm, g, xk, xkq, fac)
    ...
    USE dft_setting_params, ONLY : screening_parameter, exx_fraction, exx_fraction_lr
...
!$omp parallel do default(shared), private(ig,qq)
    DO ig = 1, ngm
      ...
      ELSEIF (qq > eps_qdiv  ) THEN
         !
         IF (exx_fraction_lr /= 0._DP) THEN
               fac(ig) = e2*fpi/qq*(exx_fraction+ exx_fraction_lr*EXP(-qq/4._DP/screening_parameter**2)) * grid_factor_track(ig) / exx_fraction
         ELSE
