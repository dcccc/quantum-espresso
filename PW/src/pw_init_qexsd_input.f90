...
!
  ...
  REAL(DP),TARGET                          ::  xdm_a1_, xdm_a2_, lond_s6_, lond_rcut_, ts_vdw_econv_thr_,&
                                               scr_par_, exx_frc_, exx_frc_lr, ecutvcut_, ecut_fock_, loc_thr_, cell_factor_tg      
  REAL(DP),POINTER                         ::  xdm_a1_pt, xdm_a2_pt, lond_s6_pt, lond_rcut_pt, ts_vdw_econv_thr_pt, & 
                                               ecut_fock_opt, scr_par_opt, exx_frc_opt, exx_frc_lr_opt, ecutvcut_opt, loc_thr_p,    &
                                               cell_factor_pt
  ...
  NULLIFY (ecut_fock_opt, scr_par_opt, exx_frc_opt, exx_frc_lr_opt, ecutvcut_opt)  
  ...
  IF ( dft_is_hybrid) THEN
     ...
     END IF 
     IF ( exx_fraction_lr > 0.0_DP) THEN 
        exx_frc_lr = exx_fraction_lr 
        exx_frc_lr_opt => exx_frc_lr
     END IF 
     ...
     CALL qexsd_init_hybrid(hybrid_, dft_is_hybrid, NQ1 = ip_nqx1, NQ2= ip_nqx2, NQ3=ip_nqx3,&
                            ECUTFOCK = ecut_fock_opt, EXX_FRACTION = exx_frc_opt, EXX_FRACTION_LR = exx_frc_lr_opt, &
