...
!----------------------------------------------------------------------------
  ...
  CONTAINS
    ...
    SUBROUTINE pw_write_schema( only_init, wf_collect )
      ...
      USE dft_setting_params,   ONLY : exx_fraction_lr
...
!-------------------------------------------------------------------------------
         ...
         IF (xclib_dft_is('hybrid') ) THEN 
            ...
            CALL qexsd_init_hybrid(hybrid_obj_opt, DFT_IS_HYBRID = .TRUE., NQ1 = nq1 , NQ2 = nq2, NQ3 =nq3, & 
                                   ...
                                   EXX_FRACTION = xclib_get_exx_fraction(), EXX_FRACTION_LR= exx_fraction_lr, SCREENING_PARAMETER = scr_par_pt, &
...
!$omp end parallel
    ...
    SUBROUTINE read_xml_file ( wfc_is_collected )
      ...
      REAL(dp) :: exx_fraction, exx_fraction_lr, screening_parameter
      TYPE (output_type)        :: output_obj 
      ...
      CALL qexsd_copy_dft ( output_obj%dft, nsp, atm, &
           dft_name, nq1, nq2, nq3, ecutfock, exx_fraction, exx_fraction_lr, screening_parameter, &
      ...
      IF ( xclib_dft_is('hybrid') ) THEN
         ...
         ecutfock = ecutfock*e2
         CALL xclib_set_exx_fraction( exx_fraction, exx_fraction_lr ) 
